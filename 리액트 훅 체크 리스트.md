...
- [ ] useGetUserByUsername - 사용자명으로 관리자 조회
...
- [ ] useGetBlogs - 내 블로그 목록 조회
- [ ] useGetBlog - 블로그 상세 조회
- [ ] useGetBlogBySlug - 슬러그로 블로그 조회
- [ ] useCreateBlog - 블로그 생성
- [ ] useUpdateBlog - 블로그 수정
- [ ] useDeleteBlog - 블로그 삭제
- [ ] useGetBlogStats - 블로그 통계 조회
- [ ] useUpdateBlogStatus - 블로그 활성화/비활성화
- [ ] useUpdateBlogVisibility - 블로그 공개/비공개 설정

## 5. 포스트(Post) 훅

- [ ] useGetPosts - 포스트 목록 조회
- [ ] useGetBlogPosts - 특정 블로그 포스트 목록 조회
- [ ] useGetPost - 포스트 상세 조회
- [ ] useGetPostBySlug - 슬러그로 포스트 조회
- [ ] useCreatePost - 포스트 작성
- [ ] useCreateBlogPost - 특정 블로그에 포스트 작성
- [ ] useUpdatePost - 포스트 수정
- [ ] useDeletePost - 포스트 삭제
- [ ] useSearchPosts - 포스트 검색
- [ ] useSearchBlogPosts - 특정 블로그에서 포스트 검색
- [ ] usePublishPost - 포스트 발행/취소
- [ ] usePostViews - 포스트 조회수 관련
- [ ] usePostLikes - 포스트 좋아요 관련
- [ ] useDraftPost - 임시 저장
- [ ] useGetDrafts - 임시 저장 목록 조회
- [ ] useGetBlogDrafts - 특정 블로그의 임시 저장 목록 조회
- [ ] useRestoreDraft - 임시 저장본 복구
- [ ] useAutosavePost - 자동 저장
- [ ] useBatchPosts - 일괄 작업
- [ ] useRelatedPosts - 관련 포스트

## 6. 카테고리(Category) 훅 - 계층 구조

- [ ] useGetCategories - 카테고리 목록 조회 (계층 구조)
- [ ] useGetBlogCategories - 특정 블로그 카테고리 목록 조회
- [ ] useGetCategoriesFlat - 카테고리 평면 목록 조회
- [ ] useGetCategory - 카테고리 상세 조회
- [ ] useGetCategoryBySlug - 슬러그로 카테고리 조회
- [ ] useCreateCategory - 카테고리 생성
- [ ] useCreateBlogCategory - 특정 블로그에 카테고리 생성
...
- [ ] useGetBlogHashtags - 특정 블로그 해시태그 목록 조회
- [ ] useCreateHashtag - 해시태그 생성
- [ ] useCreateBlogHashtag - 특정 블로그에 해시태그 생성
- [ ] useUpdateHashtag - 해시태그 수정
- [ ] useDeleteHashtag - 해시태그 삭제
- [ ] useGetHashtagPosts - 해시태그별 포스트
- [ ] useHashtagAutocomplete - 해시태그 자동완성
- [ ] useBlogHashtagAutocomplete - 특정 블로그 해시태그 자동완성

## 8. 포스트 조회/좋아요(PostView/PostLike) 훅
...
- [ ] useGetBlogViewsStats - 특정 블로그 조회수 통계 조회
- [ ] useGetBlogLikesStats - 특정 블로그 좋아요 통계 조회
...
- [ ] useGetBlogComments - 특정 블로그 댓글 목록 조회
...
- [ ] useBlogUploadImage - 특정 블로그에 이미지 업로드
- [ ] useGetBlogImages - 특정 블로그 이미지 목록 조회
...
- [ ] useBlogStats - 블로그별 통계
- [ ] useBlogAnalytics - 블로그 분석 데이터
- [ ] useBackup - 백업/복원
- [ ] useRestore - 데이터 복원
...
- [ ] useSitemap - 사이트맵 생성
- [ ] useRSSFeed - RSS 피드 생성
- [ ] useBlogSitemap - 특정 블로그 사이트맵
- [ ] useBlogRSSFeed - 특정 블로그 RSS 피드

## 14. 공개 블로그(Public Blog) 훅

- [ ] useBlogMain - 블로그 메인 (전체 블로그 목록으로 변경 예정)
- [ ] useBlogPosts - 공개 포스트 목록 (전체 블로그 포스트로 변경 예정)
- [ ] useBlogPost - 공개 포스트 상세
- [ ] useBlogCategories - 공개 카테고리 (전체 블로그 카테고리로 변경 예정)
- [ ] useBlogSearch - 블로그 검색 (전체 블로그 검색으로 변경 예정)
- [ ] useBlogAbout - 블로그 소개 (플랫폼 소개로 변경 예정)

## 15. 개별 블로그 공개(Individual Blog) 훅

- [ ] useIndividualBlogMain - 개별 블로그 메인
- [ ] useIndividualBlogPosts - 개별 블로그 포스트 목록
- [ ] useIndividualBlogPost - 개별 블로그 포스트 상세
- [ ] useIndividualBlogCategories - 개별 블로그 카테고리
- [ ] useIndividualBlogSearch - 개별 블로그 검색
- [ ] useIndividualBlogAbout - 개별 블로그 소개
...
- [ ] useBlogContext - 현재 블로그 컨텍스트 관리
- [ ] useMarkdownEditor - 마크다운 에디터 상태 관리
- [ ] useAutosave - 자동 저장 (범용)
